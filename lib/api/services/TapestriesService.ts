...
export class TapestriesService extends Service {
   ...
   * @return Promise
   */
  create (modelName, values, options: {[key: string]: any} = {}) {
    const Model = this.app.orm[modelName] || this.app.spools.waterline.orm.collections[modelName]
   ...
   * @return Promise
   */
  find (modelName, criteria, options: {[key: string]: any} = {}) {
    const Model = this.app.orm[modelName] || this.app.spools.waterline.orm.collections[modelName]
    const modelOptions = _.defaultsDeep({ }, options,
        this.app.config.get('tapestries.models.options'))
    let query
  ...
  createAssociation (parentModelName, parentId, childAttributeName, values, options = {}) {
    const parentModel = this.app.orm[parentModelName] ||
        this.app.spools.waterline.orm.collections[parentModelName]
  ...
  findAssociation (parentModelName, parentId, childAttributeName, criteria, options) {
    ...
    const childModel = this.app.orm[childModelName] ||
        this.app.spools.waterline.orm.collections[childModelName]
